...
<div>
    <header>
        <div style="height:50px;width:100%;background-color: #FC605A">
            ...
            <span style="width: 100%;height: 50px;line-height: 50px;font-size: 20px;color:#ffffff;margin-left:40%">查看班级</span>
        </div>
    </header>
       		...
       		<c:choose>
       			...
       			<c:otherwise>
				     <c:forEach var="item" items="${zclasses }">
				 		    <a href="${APP_PATH }/attend/selectAttendByCid.do?id=${item.zcid}">
				                 <span class="user-name"> ${item.zcname }</span>
				                  <span style="font-size: 12px">(点击查看签到情况)</span>
				             </a>
				 		<br>
				 	</c:forEach>
				 </c:otherwise>
