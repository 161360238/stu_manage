...
<body>
 <div class="x-nav">
    ...
    <div class="x-body">
      <div class="layui-row">
        <form  id="seacherInfo" class="layui-form layui-col-md12 x-so" action="${baseurl}stuBasicInfoList.do" method="POST">       
           <label for="addr-show">
      ...
      <table  class="layui-table">
        ...
        <tbody>
	        ...
	        <tr>
	            ...
	            <td>${row.cname}</td>
	            <td>${row.rid}</td>          
	            <td>${row.student2.role}</td>          
	            <td> <a  onclick=x_admin_show('${row.sname}个人信息',"${baseurl}selectStuDetailInfo.do?sid=${row.sid}",800,500) href="javascript:;">
	               <i>详情</i>
	              </a>
	              </td>          	            
	            <td class="td-manage">                
	              <a  onclick=x_admin_show('添加${row.sname}个人信息',"${baseurl}snedStuInfoBymanager.do?sid=${row.sid}",550,500) href="javascript:;">
	               <i class="layui-icon">&#xe642;</i>
	              </a>
	               <a title="编辑学生奖惩信息"  onclick=x_admin_show('添加${row.sname}个人荣誉',"${baseurl}sendRewardSid.do?sid=${row.sid}",550,500) href="javascript:;">
	                <i class="layui-icon">&#xe605;</i>
	              </a>
	              <a  title="编辑学生奖惩信息" onclick=x_admin_show('添加${row.sname}处分',"${baseurl}sendPunishSid.do?sid=${row.sid}",550,500) href="javascript:;">
       ...
       <itpage:page url="${baseurl}stuBasicInfoList.do"></itpage:page>
    </div>
    ...
    $(function(){  
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
        	  $.ajax({
        		  url:'${baseurl}delectStuByManager.do',
        		  type:'POST',
        		  data:{sid:id},
        		  success:function(result){
